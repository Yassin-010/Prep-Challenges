...
const cvFormatter = (arr) => {
    ...
    for (let i = 0; i < arr.length; i++) {
        ...
        if (yearsOfExperience > 1) {
            ...
            formattedCvs.push({ fullName, tech });
        }
    }
...
const applicationsStatics = (arr) => {
    ...
    for (let i = 0; i < arr.length; i++) {
        ...
        if (!firstName || !lastName && yearsOfExperience <= 1) {
...
const classesAvg = (data) => {
    for (let i = 0; i < data.grades.length; i++) {
        ...
        for (let j = 0; j < grade.classes.length; j++) {
            const currentClass = grade.classes[j];
            const sum = currentClass.classScores.reduce((acc, score) => acc + score, 0);
            const average = Math.floor(sum / currentClass.classScores.length);
